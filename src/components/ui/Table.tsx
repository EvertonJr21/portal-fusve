...
export function TableHead({ children }: { children: ReactNode }) {
  return <thead className="sticky top-0 z-10 bg-slate-50/95 backdrop-blur-sm">{children}</thead>
}
...
export function SortableTh<K extends string>({
  ...
  return (
    <th
      className={`px-3 py-2.5 text-left text-[11px] font-bold uppercase tracking-wide text-slate-500 ${
    ...
    >
      <span className="inline-flex items-center gap-1">
        {children}
        {arrow && <span className={`transition-opacity ${active ? 'opacity-100' : 'opacity-35'}`}>{arrow}</span>}
      </span>
    </th>
  )
}
