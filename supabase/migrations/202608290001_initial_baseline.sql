...
-- Cópia da seção "SQL de migração completo" do CLAUDE.md tal como documentada
-- em 29-30/08/2026: colunas novas em ocs/sols/forns (que já existiam desde o
-- projeto vanilla, migradas manualmente pro Supabase antes da virada pro
-- React), criação de pareceres/contratos/contrato_produtos, índices, e a
-- primeira geração de RLS (que liberava tudo pro role anon — substituída por
-- 202608300001_rls_autenticado.sql no dia seguinte, NÃO é o estado atual).
--
-- Objetivo: registrar o baseline pra ter um ponto de partida versionado.
-- Impacto: nenhum — já aplicado em produção, não rodar de novo.
-- Risco: nenhum (histórico).
...
-- ── Contratos (desenho da Fase 5 — cabeçalho + produtos) ────────────────
...
-- ── RLS — geração inicial (SUBSTITUÍDA no dia seguinte, ver 202608300001) ──
-- Histórico apenas: liberava tudo pro role anon. NÃO representa o estado atual.
