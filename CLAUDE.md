...
- **Usuário novo nasce sem nenhum módulo liberado** — o admin concede
  explicitamente pela tela `/admin`. Usuário que já existia na hora da
  migration ganhou ver+editar nos 4 módulos automaticamente no backfill
  (pra não perder acesso do dia pra noite) — cabe ao admin reduzir depois se
  quiser.
