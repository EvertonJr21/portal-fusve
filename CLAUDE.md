...
> **`cnpj` (21/09/2026)**: coluna nova, aditiva (migration
> `202609210002_forns_cnpj.sql`, pendente de execução por Everton no SQL
> Editor). Vem do cadastro completo de fornecedores do SoulMV
> (`R_FORNEC.csv`, ~4.446 fornecedores) — importável pela tela
> `/ocs/cadastro-fornecedores` (ver `src/utils/fornecedoresCsv.ts`). Pedido
> do Everton pra ter todos os fornecedores reais disponíveis na escolha de
> fornecedor em outros módulos (a começar por OPME), não só os poucos
> cadastrados manualmente com e-mail/WhatsApp de cobrança.
