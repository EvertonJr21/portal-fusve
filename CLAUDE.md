...
| 23 | **Adicionar `respondido_em timestamptz` em `hist_oc`** — necessário pra ação "✓ Marcar como respondida" (item 22) funcionar contra produção; sem essa coluna, a ação falha com erro visível (não quebra o resto da tela). SQL: `ALTER TABLE hist_oc ADD COLUMN IF NOT EXISTS respondido_em timestamptz DEFAULT NULL;` | OCs | **Alta — bloqueia a ação "marcar como respondida"** | Pendente — precisa da sua confirmação pra rodar DDL em produção |
