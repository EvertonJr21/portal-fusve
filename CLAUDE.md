...
| 29 | **Hardening v1.0 — migração de datas texto→date, Fase 1 (aditiva)** | Base, OCs, Pareceres | Média | **Fase 1 pronta (16/09/2026), ainda NÃO aplicada em produção** — item de maior risco do plano de hardening, por isso segue o processo de 6 passos da seção 9 do `CLAUDE_ENGINEERING.md` em vez de `ALTER COLUMN` direto (ver seção "Migração de Datas Texto → Date" acima pro detalhe completo). 3 migrations novas (`202609160001_datas_ocs_date.sql`, `202609160002_datas_sols_date.sql`, `202609160003_datas_pareceres_date.sql`) — cada uma só adiciona coluna `_date` (`date`) e faz backfill via `to_date()` nas linhas cujo texto já bate `DD/MM/AAAA`, sem tocar nas colunas texto originais nem no código de aplicação. **Everton precisa rodar as 3 no SQL Editor e confirmar que as queries de verificação (comentadas no fim de cada arquivo) deram 0 linhas** antes de eu tocar em qualquer código — só depois disso entra a Fase 2 (trocar `date.ts`, os repositories de `ocs`/`sols`/`pareceres`, formulários e telas de exibição pra usar a coluna `date`, uma tabela por vez, e só então remover a coluna texto). Build/typecheck/testes inalterados — nenhum código de app mudou nesta etapa |
