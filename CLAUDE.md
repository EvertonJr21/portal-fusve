...
**Fase 2 (troca de código, ainda não iniciada)** — só começa depois que o Everton rodar a
Fase 1 e confirmar que as verificações deram 0. Vai exigir mudanças em `src/utils/date.ts`
(as funções que hoje trabalham com string DD/MM/AAAA), nos repositories (`ocRepository.ts`,
`solRepository.ts`, `parecerRepository.ts` — ler/escrever a coluna `_date` em vez da texto),
formulários (`input[type=date]` já usa `YYYY-MM-DD`, então a conversão pra exibição
`toInput`/`fromInput` muda de sentido), e nos parsers de importação (`csv.ts` continua
recebendo `DD/MM/AAAA` do SoulMV, só a gravação no banco muda de formato). Escopo grande
demais pra fazer de uma vez — cada tabela/coluna deve ser sua própria etapa, com testes
antes de remover a coluna texto (passo 6, só depois de confirmar que nada mais lê o texto).
