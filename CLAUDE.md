...
| 47 | **3 capacidades de admin: reatribuir dados, redefinir senha, suspender conta** | Base | Média | ✅ Código pronto (22/09/2026), **migration + 2 Edge Functions pendentes de execução por Everton**. Depois do item 46, o Everton pediu uma pesquisa de "o que um admin pode fazer" inspirada em apps consolidados (GitHub/Notion/Linear/Slack/Vercel) — apresentadas 3 sugestões de alta prioridade, confirmado "Sim, começa pelos 3". **(1) Reatribuir dados** (`reatribuirDados()`, `permissaoRepository.ts`) — move OCs/Sols/Pareceres/Contratos/OPMEs de uma conta pra outra, direto no cliente (sem Edge Function — `is_admin()` já dá bypass nas policies de `UPDATE` das 5 tabelas com `owner_id`, migration `202609220004`). Útil antes de excluir/suspender alguém que ainda é dono de registro, ou pra mover histórico quando alguém troca de setor. UI: botão "⇄" por linha em `Admin.tsx` abre um modal (`ModalReatribuir`) com um `<select>` das outras contas. **(2) Redefinir senha direto** (Edge Function nova `reset-password`, `supabase/functions/reset-password/index.ts`) — mesmo padrão de autenticação das outras functions de admin (exige `Authorization` + `profiles.role = 'admin'`), `supabaseAdmin.auth.admin.updateUserById(userId, {password})`; sem depender do fluxo de e-mail de "esqueci minha senha" (o servidor de e-mail compartilhado do Supabase já esbarrou em rate limit nesse projeto antes, item 13). UI: botão "🔑" por linha abre modal (`ModalResetarSenha`) com campo de senha nova (mín. 8 caracteres, mesma regra de `DefinirSenha.tsx`). **(3) Suspender/reativar conta** (Edge Function nova `toggle-suspensao`, `supabase/functions/toggle-suspensao/index.ts`) — alternativa mais segura que excluir de vez (útil pra afastamento/licença): usa `ban_duration` nativo do GoTrue (`'876000h'` ≈ 100 anos pra "suspenso", `'none'` pra reativar — GoTrue não tem duração "indefinida" de verdade) e espelha o estado em `profiles.suspensa` (coluna nova, migration `202609220006_suspensao_de_conta.sql`, `boolean not null default false`) pra UI mostrar o badge sem outra chamada à API de admin. Bloqueado suspender a própria conta (front e function). UI: botão "⏸"/"▶" por linha (alterna ícone conforme o estado) + badge vermelho "Suspensa" ao lado do e-mail quando `p.suspensa`. As 3 ações passaram por `useConfirm()` antes de executar, mesmo padrão do resto do projeto. `tsc -b`/`npm test` (99/99)/`npm run lint`/`npm run build` confirmados limpos. **Pendente de execução por Everton**: rodar `202609220006_suspensao_de_conta.sql` no SQL Editor; criar/colar as 2 Edge Functions novas (`reset-password` e `toggle-suspensao`) no Dashboard, mesmo processo manual já usado pras outras (Edge Functions → New Function → colar → Deploy) |
